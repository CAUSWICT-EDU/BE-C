...
// 원래는 와일드카드 임포트를 했는데 다른 분들 코드 리뷰를 보고 적절하게 수정했습니다
...
public class NumberBaseball {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            while (!Valid_choice) {
                ...
                try {
                    ...
                    Valid_choice = true; // 사용자가 제대로 입력해야지만 반복문을 빠져나감
                }
        ...
        switch (choice) { //각각의 변수를 인수로 넣어 "playGame" 함수 호출 (=게임실행)
            ...
            default:
                ...
                //사용자의 입력이 정수이지만, 1,2,3 중 하나가 아닐 경우 처리
        }
    ...
    private static void playGame(int numDigits, boolean allowLeadingZero, boolean allowDuplicates, int maxAttempts) {
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            boolean Valid_guess = false; // "guess"가 유효한, 적합한 값인지를 나타내는 변수
            ...
            while (!Valid_guess) {
                try {
                    ...
                    // "guess"의 자릿수를 검사하고 "numDigits"와 일치하지 않으면 오류메세지 생성
                    if (guess.length() != numDigits) {
                        System.out.print("답안은 자릿수가 " + numDigits + "개인 정수가 되어야 합니다.\n");
                        throw new IllegalArgumentException();
                    }

                    // "guess"의 모든 입력을 한 자리씩 검사하고 정수가 아닌 부분이 있으면 (공백포함) 오류메세지 생성
                    ...
                    // 여기까지 도달했으면 n자리 정수임이 확실하므로, 유효하다고 판정
                    Valid_guess = true;

                // 오류 메세지를 받아 플레이어에게 안내메세지를 출력한 후 다시 입력하도록 유도 (try ~~ 부분으로 돌아감)
                } catch (IllegalArgumentException e) {
                    System.out.println("올바른 정수를 다시 입력해주세요. ");
                }
            }
